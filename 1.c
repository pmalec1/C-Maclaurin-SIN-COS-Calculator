...
int main(void)
{
 ...
 int error = check_data_from_user(number_of_components_ptr,number_of_modifications_ptr,xstart_ptr,step_ptr);
 displaying_xstart = xstart;  
 for(short i = 0; i<=number_of_modifications; i++ )
  { 
    if(error != 0)
    {
    ...
    if(xstart > 2*M_PI) xstart = fmod(xstart, 2*M_PI);
    printf("sin(%.2f) = %.3f " ,displaying_xstart, maclaurin_sinus(xstart,number_of_components,array_of_factorials_0_to_limit));
    printf("\t\tcos(%.2f) = %.2f\n",displaying_xstart, maclaurin_cosinus(xstart,number_of_components,array_of_factorials_0_to_limit));
